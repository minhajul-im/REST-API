        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-6 justify-items-center transition-all duration-300">
          {PRODUCTS.map((item: ProductType) => (
            <div
              ...
              <label className="absolute text-white bg-gradient-to-r from-[#ff3a4b] to-[#112142] rounded px-3 py-1 transform -rotate-45 top-8 font-fontMed">
                ৳{item?.price}.00
              </label>
              <button
                onClick={() => handleDetails(item?.id)}
                className="absolute right-2 top-2 bg-gradient-to-r from-[#d858f8] to-[#1a2438] text-white rounded-lg text-xs font-light py-1 px-2">
                details
              </button>
